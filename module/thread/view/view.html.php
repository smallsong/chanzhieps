        ...
        <td class='a-right'>
          ...
          if(!$board->readonly)
          {
              if($this->app->user->account != 'guest')
              {
                  ...
                  if($this->thread->hasManagePriv($this->app->user->account, $board->owners)) echo html::a(inlink('deleteThread', "threadID=$thread->id"), $lang->delete, 'hiddenwin');
              }    
              else
              {
        ...
        <td class='a-right'>
          ...
          if(!$board->readonly)
          {
              if($this->app->user->account != 'guest')
              {
                  ...
                  if($this->thread->hasManagePriv($this->app->user->account, $board->owners)) echo html::a(inlink('deleteReply', "replyID=$reply->id"), $lang->delete, 'hiddenwin');
              }
              else
              {
                  echo html::a($this->createLink('user', 'login', 'referer=' . helper::safe64Encode($this->app->getURI(true))) . '#reply', $lang->reply->common);;
